...
/**
 *    \file       htdocs/custom/fraispro/core/modules/fraispro/mod_fraisproreceipt_standard.php
 *    \ingroup    fraispro
 ...
 */
class mod_fraisproreceipt_standard extends ModeleNumRefFraispro
{
	/**
	...
	public function canBeActivated($object)
	{
		...
		$sql .= " FROM ".MAIN_DB_PREFIX."fraispro_receipt";
		...
		if ($resql) {
			...
			if ($row) {
				$rryymm = substr($row[0], 0, 6);
				$max = $row[0];
			}
		}

		if (!$rryymm || preg_match('/'.$this->prefix.'[0-9][0-9][0-9][0-9]/i', $rryymm)) {
			return true;
		} else {
			$langs->load("errors");
	 ...
	 *  Return next value
	 *
	 *  @param  CommonObject $object      Object commercial proposal
	 ...
	 *
	 *  @param  CommonObject $object       Object for number to search
